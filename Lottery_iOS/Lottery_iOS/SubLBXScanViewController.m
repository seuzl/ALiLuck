...
- (void)showNextVCWithScanResult:(LBXScanResult*)strResult
{
    ...
    NSString *activity_id = strResult.strScanned;
    NSString *longitude = [NSString stringWithFormat:@"%f",[Config getLongitude]];
    NSString *latitude = [NSString stringWithFormat:@"%f",[Config getLatitude]];

    
    if ([Config checkConnect]) {
        
            NSString* gameurl = [NSString stringWithFormat:@"%@?user_id=%@&access_token=%@&activity_id=%@&longitude=%@&latitude=%@&reverse_id=%@&reverse_token=%@&type=1",[Config getQingWuURL],user_id,access_token,activity_id,longitude,latitude,@"",@""];
            GameVC* gameVC = (GameVC*)[Config getVCFromSb:@"GameVC"];
            gameVC.gameURL = gameurl;
        ...
        if ([Config checkBlue]) {
            if ([Config getConnectManager].session.connectedPeers.count) {
                
                NSLog(@"游戏无网：转发中...");
                NSDictionary* params = @{@"requestFlag":@1,@"user_id":user_id,@"activity_id":activity_id,@"longitude":longitude,@"latitude":latitude,@"access_token":access_token};
                
                AppDelegate* appDelegate = (AppDelegate*)[UIApplication sharedApplication].delegate;
                [appDelegate.connectManager askOthersHelpSendData:params];
                
                [Config showProgressHUDwithStatus:@"努力加载中..."];
                
                appDelegate.feedBackBlock = ^(NSDictionary*dic){
                    
                    if ([dic[@"success"] boolValue]) {
                        [dic[@"gameData"] writeToFile:[Config getGameFilePathWithMyResult:dic[@"myResult"] gameResults:dic[@"gameResults"]] atomically:YES];
                        GameVC* gameVC = (GameVC*)[Config getVCFromSb:@"GameVC"];
                        gameVC.gameURL = nil;
                        gameVC.myResult = dic[@"myResult"];
                        gameVC.gameResults = dic[@"gameResults"];
                        [Config dismissHUD];
                        [self.navigationController pushViewController:gameVC animated:YES];
                        
                    } else {
                        [Config showErrorHUDwithStatus:@"别人也无能为力啦，请稍后再试!"];
            ...
            [Config showErrorHUDwithStatus:@"网络无法连接，建议打开蓝牙试试~"];
        }
    }
}
